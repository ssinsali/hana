...
def _read_csv_bytes(file_bytes: bytes) -> pd.DataFrame:
    """UTF-8 / CP949(엑셀 한글 CSV) 등 인코딩을 순서대로 시도."""
    ...
    for enc in encodings:
        try:
            ...
            # 인코딩은 맞았지만 파서가 실패한 경우는 그대로 전달
            # (단, utf 계열 실패 후 cp949로 재시도는 계속)
            msg = str(e).lower()
    ...
    if last_error is not None:
        raise ValueError(
            "CSV 인코딩을 확인할 수 없습니다. "
            "엑셀에서 'CSV UTF-8'로 저장하거나, 기존처럼 CP949(한글 Windows) CSV를 사용해 주세요."
        ) from last_error
...
def load_event_log_from_csv(csv_path: Path) -> tuple[list[dict], list[str]]:
    df = _read_csv_bytes(csv_path.read_bytes())
